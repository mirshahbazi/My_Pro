...
public class CartNumberHelper {
     ...
     * @Description: 负责商品总量的数据显示 当传入TextView为null的时候 可以在子线程调用
     * @date 2016/12/30 11:01
     * @author 陈文豪
     */
    public static void cartSumCount(TextView textView, int sum) {
        if (textView != null) {
            isShowAndHide(textView, sum);
            textView.setText(String.valueOf(sum));
        }
        WizarPosApp.getmPersonalInfo().setCartGoodsNum(sum);
    }
    ...
    /**
     * @Title: getCartNumber
     * @return int 购物车商品总量 如果从服务器获取失败 就返回本地购物车数量
     * @Description: 获取服务器购物车中商品的数量
     * @date 2017/1/18 10:30
     * @author 陈文豪
     */
    public static int getCartNumber() throws IOException {
        ...
        HashMap<String, Object> map = GsonUtil.parseJsonToMapObject(string);
        if (map == null) {
            Log.e("error", string);
            return WizarPosApp.getmPersonalInfo().getCartGoodsNum();
        }
        Map<String, Double> result = (Map<String, Double>) map.get("result");

        Double cartGoodsNum = (Double) result.get("cartGoodsNum");
        return cartGoodsNum.intValue();
    }
