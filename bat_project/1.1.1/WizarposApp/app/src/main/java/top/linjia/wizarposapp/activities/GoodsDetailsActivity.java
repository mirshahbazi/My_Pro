    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.activity_goods_details_cartadd:
                ...
                int curCartNum = userDao.queryGoodsNum(goodsname);  //耗时操作
                user_cartInfos = userDao.queryCart(WizarPosApp.getmPersonalInfo().getResult().getId());
                if (user_cartInfos.size() != 0) {
                    /**
                     * 根据商品名称遍历购物车，返回布尔类型的标志用来判别购物车中是否已包含该种商品
                     * */
                    boolean hadgoods = cartTraverse(user_cartInfos, goodsname);
                    if (hadgoods) {
                        if (goodsNum + curCartNum <= store) {
                            //限定商品数量
                            if (praise < store && moq < praise) {
                                if (goodsNum + curCartNum <= praise) {
                                    ...
                                    if (flag) {
                                        ...
                                        new Thread() {
                                            ...
                                            public void run() {
                                                goodsAllNum = userDao.queryGoodsAllNum(WizarPosApp.getmPersonalInfo().getResult().getId());
                                                cartNum.post(new Runnable() {
                                                    @Override
                                                    public void run() {
                                                        Log.i("test", "购物车中之前含此商品，现已加入" + goodsAllNum);
                                                        cartNum.setVisibility(View.VISIBLE);
                                                        cartNum.setText(String.valueOf(goodsAllNum));
                                                    }
                                                });
                                        ...
                                        MyToast.showToast(getResources().getString(R.string.goods_purchase_addcart));
                                    } else {
                                        MyToast.showToast(getResources().getString(R.string.goods_purchase_addfaild));
                                    }
                                } else {
                                    MyToast.showToast(getResources().getString(R.string.goods_purchase_morepriase));
                                }
                            }//不限定商品数量
                            else if (praise <= 0 || praise > store || praise < moq) {
                                boolean flag = userDao.insertCart(info);
                                Log.i("test", "往数据库中添加是否成功：" + flag);
                                if (flag) {
                                    //cartnum+=1;
                 ...
                 * */
                    ...
                    //MyToast.showToast(getResources().getString(R.string.goods_purchase_wrong));
                }
                break;
